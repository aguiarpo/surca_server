...
public class IncidentEndpoint {
    ...
    @GetMapping(path = "/admin/incidentes")
    public ResponseEntity<?> listAll(Pageable pageable) {
        return new ResponseEntity<>(incidentDao.findAll(pageable), HttpStatus.OK);
    }

    @GetMapping(path = "/admin/incidentes/{id}")
    public ResponseEntity<?> getIncidentById(@PathVariable("id")Long id){
        return new ResponseEntity<>(incidentDao.findById(id), HttpStatus.OK);
    }
